...
def process_trades_upsert_trade_log(df, TRADE_LOG_PATH, all_stocks_data):
  ...
  for ticker in tickers_to_close:
      trade_id = trade_log_open_only[(trade_log['ticker'] == ticker)]['trade_id'].values[0]      
...
earnings_start_date = end_date - datetime.timedelta(days=7)

# Download stock data
tickers = get_tickers()
# print(tickers)
# ticker_data = Ticker(tickers, asynchronous=True)
ticker_data = Ticker("MRNA QQQ", asynchronous=True)
...
# Fetch earnings
earnings_calendar = fetch_earnings(earnings_start_date.date(), end_date.date())
  ...
  for date, earnings_data in earnings_calendar["earnings"].items():
    ...
    for stock in earnings_data["stocks"]:
      if stock["symbol"] == ticker and ticker in tickers:
        # We found an earnings event for this ticker, so we don't want to keep it
        keep_ticker = False
        break  # No need to check the rest of the stocks for this day

    if not keep_ticker:
      # We found an earnings event, so no need to check the rest of the days
      break

  if keep_ticker:
    # We didn't find any earnings events for this ticker, so we want to keep it
...
daily_return = {}
daily_return["QQQ"] = data["close"]["QQQ"].pct_change()
...
for ticker in tickers:
    ...
    try:
        ...
        condition3 = daily_return[ticker].mean() > daily_return["QQQ"].mean()
        ...
        if len(ticker_highs[ticker]) > 1 and \
          ...
          daily_return[ticker].mean() > daily_return["QQQ"].mean() and \
          all_stocks_data["close"][ticker].iloc[0] > 5 and \
...
screener_log = upsert_screener_log(df, daily_return['QQQ'], all_stocks_data)
